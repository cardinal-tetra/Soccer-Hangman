...
USER_REQUEST = endpoints.ResourceContainer(
    username=messages.StringField(1, required=True), email=messages.StringField(2)
)

NEW_GAME = endpoints.ResourceContainer(username=messages.StringField(1, required=True),)
...
LIMIT = endpoints.ResourceContainer(number_of_results=messages.IntegerField(1),)
...
class soccerhangman(remote.Service):
    ...
    @endpoints.method(
        USER_REQUEST, StringMessage, path="user", name="create_user", http_method="POST"
    )
    def create_user(self, request):
        ...
        return StringMessage(
            message="We have successfully registered %s!" % request.username
        )

    @endpoints.method(
        NEW_GAME, GameForm, path="game", name="new_game", http_method="POST"
    )
    ...
    def get_user_games(self, request):
        ...
        # find the games
        games = (
            Game.query(Game.username == user.key)
            .filter(Game.game_status == "ongoing")
            .fetch()
        )
        if not games:
        ...
        return GameForms(
            items=[
                game.to_form("active game", produce_hint(game.answer), "ongoing")
                for game in games
            ]
        )
    ...
    @endpoints.method(
        LIMIT,
        StringMessages,
        path="scores/highscores/{number_of_results}",
    ...
    def get_high_scores(self, request):
        ...
        items = [
            [score.user.get().username, str(score.date), str(score.guesses)]
            for score in scores
        ]
        return StringMessages(
            table=[StringMessage(message=", ".join(item)) for item in items]
        )

    @endpoints.method(
        LIMIT,
        StringMessages,
        path="user/rankings/{number_of_results}",
    ...
    def get_user_rankings(self, request):
        ...
        users = (
            User.query().order(-User.win_ratio).order(User.average_guesses).fetch(limit)
        )
        # generate and return rankings
        items = [
            [
                user.username,
                "win ratio " + str(user.win_ratio),
                "average guesses " + str(user.average_guesses),
            ]
            for user in users
        ]
        return StringMessages(
    ...
    def get_game_history(self, request):
        ...
        items = game.history
        return StringMessages(
            table=[StringMessage(message=str(item)) for item in items]
        )
